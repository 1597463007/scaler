...
from scaler.protocol.python.message import StateScheduler, InformationRequest, InformationSnapshot
...
class VanillaInformationController(InformationController, Looper):
    ...
    async def routine(self):
        ...
        await self._scaling_controller.on_snapshot(
            InformationSnapshot(
                tasks=self._task_controller._task_id_to_task,
                workers={
                    worker_id: worker_heartbeat
                    for worker_id, (_, worker_heartbeat) in self._worker_controller._worker_alive_since.items()
                },
            )
        )
